...
const consumo = (objMenu, string) => {
  // recebe o obejeto menu e adiciona a string passada no array consumption
  objMenu.consumption.push(string);
};
...
const createMenu = (objeto) => {
  const objMenu = {
    fetchMenu: () => objeto,
    consumption: [],
    order: (string) => consumo(objMenu, string),
    pay: () => {
      // a soma dos valores
      let valor = 0;
      // um forEach para percorer nas duas chaves do array 
      objMenu.consumption.forEach((item) => {
        // console.log(item);
        if (Object.keys(objeto.food).includes(item)) {
          valor += objeto.food[item];
        }
        if (Object.keys(objeto.drink).includes(item)) {
          valor += objeto.drink[item];
        }
      });
      // pega o valor somado e faz a porcentagem de 10%
      let porcentagem = (valor * 0.1);
      // e retorna o valor somando a porcentagem
      valor += porcentagem;
      return valor;
    },
  };
